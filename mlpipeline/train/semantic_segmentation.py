...
class SemanticSegmentation(MLPipeline):
    ...
    def infer(self, get_images=False, get_metrics=False):
        ...
        if get_metrics:
            self.metrics_collector.reset()
        # Print
        self.logger.info("Training finished. Running Inference on the test dataset!!")
        ...
        with torch.no_grad():
            for i, batch in enumerate(test_loader):
                ...
                img_input = batch["input"].cuda(self.local_rank, non_blocking=True)
                ...
                logits = self.model.predict(batch, inferer=inferers[dataset_name])["pred"]
                ...
                torch.save(
                    logits,
                    Path(self.cfg.inference_dir) / dataset_name / f"{input_image_name}.pt",
                )
                ...
                if get_images:
                    ...
                    cv2.imwrite(str(Path(self.cfg.inference_dir) / dataset_name / f"{input_image_name}.png"), out)
